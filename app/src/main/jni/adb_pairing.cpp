...
static jbyteArray PairingContext_Encrypt(JNIEnv *env, jobject, jlong ptr, jbyteArray jIn) {
    ...
    auto in_buf = env->GetByteArrayElements(jIn, nullptr);
    ...
    int status = EVP_AEAD_CTX_seal(aes_ctx, out, &written_sz, out_size, nonce, nonce_size,
                                    (uint8_t *) in_buf, in_size, nullptr, 0);
    env->ReleaseByteArrayElements(jIn, in_buf, 0);

    if (!status) {
        LOGE("Failed to encrypt (in_len=%d)", in_size);
        return nullptr;
    }
    ++ctx->enc_sequence;
...
static jbyteArray PairingContext_Decrypt(JNIEnv *env, jobject, jlong ptr, jbyteArray jIn) {
    ...
    if (!status) {
        LOGE("Failed to decrypt (in_len=%d)", in_size);
        return nullptr;
    }
    ++ctx->dec_sequence;
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *) {
    ...
    jclass clazz = env->FindClass(kPairingContextClass);
    if (clazz == nullptr) {
        LOGE("Failed to find class %s", kPairingContextClass);
        return -1;
    }
