package com.tutu.meowhub.core.socket
...
class TutuSocketClient(
    ...
    private val scope = CoroutineScope(
        SupervisorJob() + Dispatchers.IO + CoroutineExceptionHandler { _, e ->
            Log.w(TAG, "Unhandled coroutine error (connection may have failed): ${e.message}")
        }
    )
    ...
    private val ctx get() = MeowApp.instance
    ...
    fun nextReqId(): String = "meow-${reqIdCounter.incrementAndGet()}"
    ...
    fun connect(token: String?, force: Boolean = false) {
        ...
        scope.launch {
            try {
                ...
                val hint = ctx.getString(R.string.connection_refused_hint)
                Log.w(TAG, "Cannot connect to $host:$port - $hint")
                ...
                _connectionState.value = ConnectionState.DISCONNECTED
                withContext(Dispatchers.Main.immediate) {
                    Toast.makeText(ctx, ctx.getString(R.string.connection_refused_hint), Toast.LENGTH_SHORT).show()
                }
            }
        }
    }
