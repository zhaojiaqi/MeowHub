...
class AdbKey(private val adbKeyStore: AdbKeyStore, name: String) {
    ...
    companion object {
        ...
        private const val ENCRYPTION_KEY_ALIAS = "_meowhub_adbkey_encryption_"
    ...
    init {
        encryptionKey = getOrCreateEncryptionKey()
            ?: error("Failed to generate encryption key with AndroidKeyStore.")
    ...
    private fun getOrCreatePrivateKey(): RSAPrivateKey {
        ...
        if (ciphertext != null) {
            try {
                ...
                privKey = keyFactory.generatePrivate(PKCS8EncodedKeySpec(plaintext)) as RSAPrivateKey
            } catch (_: Exception) {
            }
        }
        if (privKey == null) {
            ...
            privKey = keyPair.private as RSAPrivateKey

            val encrypted = encrypt(privKey.encoded, aad)
            if (encrypted != null) adbKeyStore.put(encrypted)
        }
        return privKey
    }
...
class PreferenceAdbKeyStore(private val preference: SharedPreferences) : AdbKeyStore {
    private val preferenceKey = "meowhub_adbkey"
...
private fun BigInteger.toAdbEncoded(): IntArray {
    val encoded = IntArray(AdbKey.ANDROID_PUBKEY_MODULUS_SIZE_WORDS)
    val r32 = BigInteger.ZERO.setBit(32)
    var tmp = this.add(BigInteger.ZERO)
    for (i in 0 until AdbKey.ANDROID_PUBKEY_MODULUS_SIZE_WORDS) {
...
private fun RSAPublicKey.adbEncoded(name: String): ByteArray {
    ...
    val r = BigInteger.ZERO.setBit(AdbKey.ANDROID_PUBKEY_MODULUS_SIZE * 8)
    val rr = r.modPow(BigInteger.valueOf(2), modulus)

    val buffer = ByteBuffer.allocate(AdbKey.RSA_PUBLIC_KEY_SIZE).order(ByteOrder.LITTLE_ENDIAN)
    buffer.putInt(AdbKey.ANDROID_PUBKEY_MODULUS_SIZE_WORDS)
