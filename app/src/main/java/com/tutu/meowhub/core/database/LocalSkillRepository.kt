...
class LocalSkillRepository(private val dao: SkillDao) {
    ...
    suspend fun importFromMarket(detail: MeowSkillDetail): Long {
        return importSkill(detail, LocalSkillEntity.SOURCE_MARKET)
    }

    suspend fun importFromClipboard(detail: MeowSkillDetail): Long {
    ...
    fun toMeowSkillDetail(entity: LocalSkillEntity): MeowSkillDetail {
        return try {
            json.decodeFromString<MeowSkillDetail>(entity.rawJson)
        } catch (_: Exception) {
            MeowSkillDetail(
